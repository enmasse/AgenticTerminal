...
public sealed class Hex1bTerminalEmulator : ITerminalDisplayState, IDisposable
{
    private static readonly TimeSpan OutputDrainTimeout = TimeSpan.FromMilliseconds(250);

    internal static async Task WaitForOutputDrainAsync(Task completion, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(completion);

        var timeoutTask = Task.Delay(OutputDrainTimeout, cancellationToken);
        await Task.WhenAny(completion, timeoutTask);
    }
    ...
    public Hex1bTerminalEmulator(int columns, int rows, int scrollback = 1000)
    {
        if (columns <= 0)
        {
    ...
    private sealed class EmulatorWorkloadAdapter : IHex1bTerminalWorkloadAdapter, IAsyncDisposable
    {
        ...
        public async ValueTask QueueOutput(string chunk, CancellationToken cancellationToken)
        {
            ...
            await WaitForOutputDrainAsync(completion.Task, cancellationToken);
        }
